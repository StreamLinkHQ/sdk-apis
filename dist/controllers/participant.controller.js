...
export const getLiveStreamParticipants = async (req, res) => {
...
export const updateLiveStreamParticipant = async (req, res) => {
    const { liveStreamId } = req.params;
    ...
    try {
        ...
        if (participant.userType === 'host') {
            ...
            const otherActiveHosts = liveStream.participants.some(p => p.userType === 'host' &&
                p.id !== participant.id &&
                !p.leftAt);
