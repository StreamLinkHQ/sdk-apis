...
export const updateStreamParticipantTime = async (req, res) => {
    ...
    try {
        ...
        if (participant.userType === "host") {
            ...
            const otherActiveHosts = liveStream.participants.some((p) => p.userType === "host" && p.id !== participant.id && !p.leftAt);
...
export const updateTempHostPermissions = async (req, res) => {
    ...
    try {
        if (!roomName ||
            ...
            return res.status(400).json({
                error: "Missing required fields: room name, wallet address and participantId",
            });
        }
