...
export const getLiveStreamAgenda = async (req: Request, res: Response) => {
  const { liveStreamId } = req.params;
  try {
    if (!liveStreamId) {
      return res.status(400).json({
...
export const updateLiveStreamAgenda = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const actionEnum =
        AgendaAction[
          action.replace("&", "_") as keyof typeof AgendaAction
        ];
        if (!actionEnum) {
          return res
            .status(400)
